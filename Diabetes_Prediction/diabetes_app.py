...
with st.form(key='columns_in_form'):
    ...
    with c1:
        ...
        income = st.selectbox('What is your income?',
                            ...
                            index=defaultInput['income'])
        bmi = st.slider("What's your BMI?", 10, 99, value=defaultInput['bmi'])

    with c2:
        st.subheader('Overall Health')
        genhealth = st.selectbox('Your general health is', ['Excellent', 'Very good', 'Good', 'Fair', 'Poor'], index=defaultInput['genhlth'])
        ...
        diffwalk = st.selectbox('Do you have serious difficulty walking or climbing stairs?', ['Yes', 'No'], index=defaultInput['diffwalk'])

    with c3:
        st.subheader('Medical Conditions')
        highcolesterol = st.selectbox('A doctor, nurse, or health professional have told you have high blood cholesterol', ['Yes', 'No'], index=defaultInput['highchol'])
        colesterolcheck = st.selectbox('Have you had a cholesterol check in the past 5 years?', ['Yes', 'No'], index=defaultInput['cholcheck'])
        stroke = st.selectbox('Have you ever been told you had a stroke', ['Yes', 'No'], index=defaultInput['stroke'])
        coronary = st.selectbox('Do you have any coronary heart disease or have had a myocardial infarction', ['Yes', 'No'], index=defaultInput['heartdiseaseorattack'])

    with c4:
        st.subheader('Other questions')
        bloodpress = st.selectbox('A doctor, nurse, or health professional have told you have high blood pressure', ['Yes', 'No'], index=defaultInput['highbp'])
